...
const FOLDER_STYLES: Record<string, { bg: string; text: string; border: string; badge: string }> = {
  Necessidades: { 
    bg: "bg-blue-50", 
    text: "text-blue-600", 
  ...
  Desejos: { 
    ...
    badge: "bg-purple-100/50 text-purple-700 hover:bg-purple-200 border-purple-200"
  },
  Poupança: { 
    bg: "bg-emerald-50", 
    text: "text-emerald-600", 
    border: "border-emerald-100",
    badge: "bg-emerald-100/50 text-emerald-700 hover:bg-emerald-200 border-emerald-200"
  },
  Investimentos: { 
    bg: "bg-emerald-50", 
    text: "text-emerald-600", 
...
const getCategoryStyle = (folderName: string | null) => {
  ...
  const key = Object.keys(FOLDER_ICONS).find((k) => name.includes(k)) || "Outros";
  return {
...
export default function ExpensesPage() {
  ...
  const lastExpenseElementRef = useCallback((node: HTMLTableRowElement | HTMLDivElement) => {
  ...
  // --- Filtros e View ---
  const [viewMode, setViewMode] = useState<"list" | "table" | "analysis">("list");
  ...
  const fetchExpenses = async (pageToLoad: number, isReset: boolean) => {
    ...
    try {
      ...
      if (filterMonth !== "all" && filterYear !== "all") {
        startDate = new Date(parseInt(filterYear), parseInt(filterMonth) - 1, 1);
        endDate = new Date(parseInt(filterYear), parseInt(filterMonth), 0, 23, 59, 59);
      ...
      if (filterCategory !== "all") {
        filteredResult = filteredResult.filter(e => e.categoryId === filterCategory);
      }
      ...
      setExpenses(prev => isReset ? filteredResult : [...prev, ...filteredResult]);
  ...
  const groupedExpenses = useMemo(() => {
    ...
    displayedExpenses.forEach((e) => {
      ...
      let key = date.toLocaleDateString("pt-BR", { day: "2-digit", month: "long", weekday: "long" });
  ...
  const totalDisplay = displayedExpenses.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const handleDelete = async (id: string) => {
    ...
    } catch (err: any) { notify.error(err, "excluir gasto"); }
  };
    ...
    <div className="min-h-screen bg-gray-50/50 p-4 pb-20 animate-in fade-in">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Extrato Financeiro</h1>
            <p className="text-slate-500 text-sm flex items-center gap-2 mt-1">
              ...
              <span className="text-xs">• {displayedExpenses.length} itens</span>
            </p>
          </div>
          <Button onClick={() => router.push("/expenses/new")} className="bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-200">
        ...
        <div className="sticky top-4 z-30 bg-white/90 backdrop-blur-md p-3 rounded-xl border border-slate-200 shadow-sm space-y-3">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
               <Select value={filterMonth} onValueChange={handleMonthChange}>
                 ...
                 <SelectContent>
                    ...
                    {[...Array(12)].map((_, i) => (
                      <SelectItem key={i} value={(i+1).toString()}>{new Date(0, i).toLocaleString('pt-BR', { month: 'long' })}</SelectItem>
                    ))}
                 </SelectContent>
               </Select>
               <Select value={filterYear} onValueChange={setFilterYear}>
                 <SelectTrigger className="w-[100px] bg-white"><SelectValue placeholder="Ano" /></SelectTrigger>
                 <SelectContent>
                    <SelectItem value="all">Todos</SelectItem>
               ...
               <Select value={filterCategory} onValueChange={setFilterCategory}>
                 <SelectTrigger className="w-[150px] bg-white">
                    <div className="flex items-center gap-2 truncate">
                       ...
                       <span className="truncate">{filterCategory === 'all' ? 'Categoria' : categories.find(c => c.id === filterCategory)?.name}</span>
                    </div>
                 </SelectTrigger>
                 <SelectContent>
          ...
          <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as any)} className="w-full">
            <TabsList className="grid w-full grid-cols-3 h-9 bg-slate-100/50 p-1">
              <TabsTrigger value="list" className="text-xs font-medium">Timeline</TabsTrigger>
              <TabsTrigger value="table" className="text-xs font-medium">Tabela</TabsTrigger>
              <TabsTrigger value="analysis" className="text-xs font-medium">Análise</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
                  ...
                  Object.entries(groupedExpenses).map(([dateLabel, items], groupIndex) => (
                    <div key={dateLabel} className="space-y-3">
                      ...
                      {items.map((expense, index) => {
                        const { icon: Icon, theme } = getCategoryStyle(expense.category?.budgetCategoryName || null);
                        const isLastItem = groupIndex === Object.entries(groupedExpenses).length - 1 && index === items.length - 1;

                        return (
                          <div 
                            key={expense.id}
                            ref={isLastItem ? lastExpenseElementRef as any : null}
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${theme.bg} ${theme.text} border ${theme.border} transition-colors`}>
                                  <Icon className="w-6 h-6" />
                                </div>
                                
                                <div className="flex flex-col gap-1.5">
                                  <div className="flex items-center gap-2">
                                     <p className="font-bold text-slate-800 text-base">{expense.description}</p>
                                     {expense.receiptUrl && (
                                       <button 
                                         onClick={(e) => { e.stopPropagation(); setSelectedReceipt(expense.receiptUrl!); }}
                                  ...
                                  <div className="flex flex-wrap items-center gap-2 text-xs">
                                    <Badge variant="outline" className={`font-medium border px-2 py-0.5 ${theme.badge} border-transparent`}>
                                    ...
                                    {expense.isInstallment && (
                                      <span className="flex items-center gap-1 text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-md font-medium border border-purple-100">
                                        <CreditCard className="w-3 h-3" /> {expense.installmentNumber}/{expense.totalInstallments}
                                      </span>
                                    )}
                                    {expense.isRecurring && (
                              ...
                              <div className="flex flex-col items-end gap-2">
                                ...
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 hover:text-slate-600 -mr-2" onClick={e => e.stopPropagation()}>
                                      <MoreHorizontal className="w-5 h-5" />
                                    </Button>
                                  </DropdownMenuTrigger>
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => router.push(`/expenses/${expense.id}/edit`)}><Edit className="w-4 h-4 mr-2" /> Editar</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => handleDelete(expense.id)} className="text-red-600"><Trash2 className="w-4 h-4 mr-2" /> Excluir</DropdownMenuItem>
                                  </DropdownMenuContent>
                                </DropdownMenu>
                              </div>
                            </div>
                     ...
                     <Table>
                        <TableHeader className="bg-slate-50 border-b border-slate-100">
                           <TableRow className="hover:bg-transparent">
                              <TableHead className="w-[140px] font-semibold text-xs uppercase tracking-wider text-slate-500">Data</TableHead>
                              <TableHead className="font-semibold text-xs uppercase tracking-wider text-slate-500">Descrição</TableHead>
                              <TableHead className="font-semibold text-xs uppercase tracking-wider text-slate-500">Categoria</TableHead>
                              <TableHead className="text-center w-[80px] font-semibold text-xs uppercase tracking-wider text-slate-500">Nota</TableHead>
                              <TableHead className="text-right font-semibold text-xs uppercase tracking-wider text-slate-500">Valor</TableHead>
                        ...
                        <TableBody>
                           {displayedExpenses.map((expense, index) => {
                              ...
                              const { theme } = getCategoryStyle(expense.category?.budgetCategoryName || null);
                                ...
                                >
                                  <TableCell className="text-slate-500 font-medium text-xs whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                      ...
                                      {new Date(expense.date.replace(/-/g, "/")).toLocaleDateString()}
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                    <div className="flex flex-col">
                                      <span className="font-semibold text-slate-700 text-sm group-hover:text-blue-700 transition-colors">{expense.description}</span>
                                      {expense.isInstallment && (
                                        <span className="text-[10px] text-purple-600 font-bold flex items-center gap-1 mt-0.5">
                                           <CreditCard className="w-2.5 h-2.5" /> Parcela {expense.installmentNumber}/{expense.totalInstallments}
                                        </span>
                                      )}
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                      <Badge variant="outline" className={`font-medium border border-transparent ${theme.badge} py-1`}>
                                        {expense.category?.name}
                                      </Badge>
                                  </TableCell>
                                  ...
                                  <TableCell>
                                     <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 opacity-0 group-hover:opacity-100 hover:text-blue-600 transition-all" onClick={() => router.push(`/expenses/${expense.id}/edit`)}>
            ...
            {viewMode === "analysis" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="border-slate-200 shadow-sm">
                  <CardHeader><CardTitle className="text-lg">Distribuição (Visível)</CardTitle></CardHeader>
                  <CardContent className="space-y-6">
                    {budgetCategories.map(bc => {
                      const catsInFolder = categories.filter(c => c.budgetCategoryId === bc.id).map(c => c.id);
                      const spent = displayedExpenses
                        .filter(e => catsInFolder.includes(e.categoryId))
                        .reduce((sum, e) => sum + e.amount, 0);
                      
                      if (spent === 0) return null;
                      const percentage = totalDisplay > 0 ? (spent / totalDisplay) * 100 : 0;
                      ...
                      return (
                        <div key={bc.id} className="group">
                           <div className="flex justify-between items-center mb-2 text-sm">
                              <div className="flex items-center gap-2">
                                <div className={`p-1.5 rounded-lg ${theme.bg} ${theme.text}`}><Icon className="w-4 h-4" /></div>
                                <span className="font-semibold text-slate-700">{bc.name}</span>
                              </div>
                              <span className="font-bold text-slate-900">R$ {spent.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                           </div>
                           <Progress value={percentage} className="h-2.5 bg-slate-100" indicatorClassName={theme.bg.replace('bg-', 'bg-').replace('50', '500')} />
                           <p className="text-xs text-slate-400 text-right mt-1 font-medium">{percentage.toFixed(1)}%</p>
                        </div>
                      )
                    })}
                  </CardContent>
         ...
         <DialogContent className="max-w-4xl p-0 overflow-hidden bg-transparent border-none shadow-none flex justify-center items-center h-[90vh]">
             <div className="relative bg-white rounded-xl overflow-hidden shadow-2xl max-h-full max-w-full flex flex-col w-full h-full md:w-auto md:h-auto">
                <div className="absolute top-2 right-2 z-50">
                  <Button variant="secondary" size="icon" className="rounded-full bg-white/90 hover:bg-white shadow-md backdrop-blur-sm" onClick={() => setSelectedReceipt(null)}>
                ...
                <div className="flex-1 overflow-auto bg-slate-100/50 p-4 flex items-center justify-center min-w-[300px] min-h-[300px]">
                   {selectedReceipt && (
                      selectedReceipt.endsWith('.pdf') ? (
                        <iframe src={selectedReceipt} className="w-[800px] h-[600px] rounded-lg shadow-inner bg-white" />
                      ) : (
                        <img src={selectedReceipt} alt="Comprovante" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-sm" />
                      )
                   )}
                </div>
                <div className="bg-white p-3 border-t border-slate-100 flex justify-between items-center">
                   <p className="text-sm font-medium text-slate-600 pl-2">Visualização do Anexo</p>
                   <Button variant="outline" size="sm" onClick={() => window.open(selectedReceipt!, '_blank')}>
...
function EmptyState() {
  return (
    <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-200 animate-in fade-in zoom-in-95">
      ...
      <h3 className="text-lg font-bold text-slate-900 mb-1">Nenhum gasto encontrado</h3>
      <p className="text-slate-500 text-sm max-w-xs mx-auto mb-6">
        Seus filtros não retornaram resultados ou você ainda não gastou nada (parabéns!).
      </p>
    </div>
  );
}