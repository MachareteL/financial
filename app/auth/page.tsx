...
// Use Cases
import { signInUseCase, signUpUseCase, resetPasswordUseCase } from "@/infrastructure/dependency-injection"
...
export default function AuthPage() {
  ...
  const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      notify.success("Bem-vindo de volta!", { description: "Login realizado com sucesso." })
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.success("Email enviado!", { description: "Verifique sua caixa de entrada para redefinir a senha." })
        ...
        <div className="relative z-10 max-w-md">
          <blockquote className="space-y-4">
            <p className="text-2xl font-medium leading-relaxed">
              "Finalmente paramos de brigar por dinheiro. O método 50/30/20 automático organizou nossa vida financeira em semanas."
            </p>
        ...
        <div className="mx-auto w-full max-w-[400px] space-y-8">
          ...
          <Tabs defaultValue="signin" className="w-full">
            ...
            <TabsContent value="signin">
              <form onSubmit={handleSignIn} className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Dialog open={isResetOpen} onOpenChange={setIsResetOpen}>
                      <DialogTrigger asChild>
                        <Button variant="link" size="sm" className="px-0 font-normal text-xs text-blue-600 h-auto">
                          Esqueceu a senha?
                        </Button>
                      </DialogTrigger>
                      <DialogContent>
                        ...
                        <div className="space-y-4 py-2">
                          ...
                          <Button onClick={handleResetPassword} disabled={isLoading} className="w-full">
                            {isLoading ? <Loader2 className="animate-spin w-4 h-4" /> : "Enviar Link"}
                          </Button>
                        </div>
                      </DialogContent>
                    </Dialog>
                ...
                <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 shadow-sm" disabled={isLoading}>
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="text-xs text-slate-500 space-y-2 py-2">
                  <p className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-green-500" /> Gestão de time familiar</p>
                  <p className="flex items-center gap-2"><CheckCircle2 className="w-3 h-3 text-green-500" /> Leitura de recibos com IA</p>
                </div>

                <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 shadow-sm" disabled={isLoading}>
                  {isLoading ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : "Criar Conta Grátis"}
                </Button>
              </form>
            </TabsContent>
          </Tabs>
          ...
          <p className="text-center text-xs text-slate-500 px-8">
            Ao continuar, você concorda com nossos <Link href="#" className="underline hover:text-slate-800">Termos de Serviço</Link> e <Link href="#" className="underline hover:text-slate-800">Política de Privacidade</Link>.
          </p>
        </div>
      </div>
    </div>
