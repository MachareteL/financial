...
export class BudgetCategoryRepository implements IBudgetCategoryRepository {
  ...
  private mapEntityToRow(entity: BudgetCategory): Omit<BudgetCategoryRow, 'id' | 'created_at'> {
     return {
  ...
  async createDefaultCategories(teamId: string): Promise<BudgetCategory[]> {
    const defaults = [
      { name: 'Necessidades', percentage: 0.50, team_id: teamId, id: crypto.randomUUID(), created_at: new Date().toISOString() },
      { name: 'Desejos', percentage: 0.30, team_id: teamId, id: crypto.randomUUID(), created_at: new Date().toISOString() },
      { name: 'Poupança', percentage: 0.20, team_id: teamId, id: crypto.randomUUID(), created_at: new Date().toISOString() },
    ];
